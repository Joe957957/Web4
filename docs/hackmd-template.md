...
### C1 程式碼已上傳到 GitHub
專案原始碼已透過 Git 推送到 GitHub，方便老師或評分者直接瀏覽。

### C2 排除 node_modules
已在 [.gitignore](../.gitignore) 中排除 `node_modules/`，避免把套件安裝目錄提交到 GitHub。

### C3 package.json 完整
專案包含 [package.json](../package.json)，並已列出 `express`、`cors`、`sqlite3` 等必要相依套件。

### C4 GitHub Repo 可瀏覽
GitHub 倉庫已設定為可透過連結瀏覽，評分者可直接開啟專案內容。

## 十一、技術說明文件

### D1 文件可貼到 HackMD 或 Notion
本文件可直接複製到 HackMD、Notion 或其他線上筆記平台，並開啟分享權限提供評分者檢視。

### D2 文件包含商品介紹與選擇理由
文件已說明為什麼選擇台鐵票價作為主題，也整理了資料來源與商品特色。

### D3 文件保留三張 localhost 截圖位置
文件已保留三張截圖的位置，分別對應首頁、新增成功與查詢結果畫面。

### D4 文件包含前端、後端、資料庫程式碼片段
文件已列出 fetch 呼叫、後端寫入資料庫，以及 SQLite 建表片段。

### D5 文件可作為教學說明參考
本文件以步驟式與說明式方式撰寫，可作為其他人重做或學習的參考。
